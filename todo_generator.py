...
# ----------------------------------------------------------------------
## 2. Fonction de génération Excel
# ----------------------------------------------------------------------
...
def generate_excel_file(month, year):
    ...
    try:
        ...
        # Ceci peut arriver si l'utilisateur entre une valeur non prévue par tkinter
        print("Erreur: Les dates fournies sont invalides.")
        return

    # Utilisation de strftime pour obtenir le nom du mois en français (dépend des paramètres locaux du système)
    try:
        month_name = start_date.strftime('%B').capitalize()
    except:
        # Fallback si les locales ne sont pas configurées pour le français
